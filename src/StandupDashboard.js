...
import { Target, AlertTriangle, Users, MapPin } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if (dateRange.startDate && dateRange.endDate) {
      filtered = filtered.filter((item) => {
        const itemDate = new Date(item.standup_date).setHours(0, 0, 0, 0); // Normaliza la fecha del dato
        const start = new Date(dateRange.startDate).setHours(0, 0, 0, 0);  // Normaliza la fecha de inicio
        const end = new Date(dateRange.endDate).setHours(23, 59, 59, 999); // Extiende la fecha de fin al final del día
        return itemDate >= start && itemDate <= end; // Compara correctamente las fechas
      });
    }
        ...
        <div className="bg-white p-6 rounded-xl shadow-md overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {["User", "Location", "Tasks/Day", "Total Commits", "Obstacles", "Avg. Satisfaction"].map((header) => (
                    <th key={header} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {header}
                    </th>
                  ))}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {getUserMetrics().map((user, index) => (
                  <tr key={index}>
                    <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{user.user}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-500">{user.location}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-500">{user.tasksPerDay}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-500">{user.totalCommits}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-500">{user.obstaclesCount}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-500">{user.avgSatisfaction}</td>
                  </tr>
                ))}
              </tbody>
            </table>
